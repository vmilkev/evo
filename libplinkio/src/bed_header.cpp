...
get_version_and_order(const unsigned char *header, enum BedVersion *version, enum SnpOrder *snp_order)
{
    if( header[ 0 ] == BED_V100_MAGIC1 && header[ 1 ] == BED_V100_MAGIC2 )
    {
        ...
        *snp_order = static_cast<enum SnpOrder>( get_snp_order( header[ 2 ] ) );
    ...
    else if( ( header[ 0 ] & ~BED_SNP_ORDER_BIT ) == 0 )
    {
        ...
        *snp_order = static_cast<enum SnpOrder>( get_snp_order( header[ 0 ] ) );
    }
    else
    {
        /* Version < 0.99 */
...
{
    struct bed_header_t header = { static_cast<enum SnpOrder>(0) };
