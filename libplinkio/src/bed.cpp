...
transpose_file(const unsigned char *mapped_file, size_t num_loci, size_t num_samples, const char *output_path)
{
    ...
    try
    {
        if( output_file == NULL ) throw;

        transposed_fd = fileno(output_file);
        if( transposed_fd < 0 ) throw;

        /* Clear size of file, otherwise we might have trailing bytes */
        if(libplinkio_ftruncate_(transposed_fd, 0) == -1) throw;
...
libplinkio_bed_transpose_fd_(const int original_fd, const int transposed_fd, size_t num_loci, size_t num_samples)
{
    ...
    try
    {
        ...
        if( fwrite( byte_header, sizeof( unsigned char ), byte_header_length, output_file ) != byte_header_length ) throw;
...
bed_transpose(const char *original_path, const char *transposed_path, size_t num_loci, size_t num_samples)
{
    ...
    #endif
        if( transposed_fd == -1 ) throw;

        /* Transpose */
        if (libplinkio_bed_transpose_fd_(original_fd, transposed_fd, num_loci, num_samples) != PIO_OK) throw;
...
pio_status_t
libplinkio_bed_transpose_pio_bed_file_(struct pio_bed_file_t *bed_file, const char *transposed_path, size_t num_loci, size_t num_samples, bool is_tmp)
{
    int original_fd = -1;
    int transposed_fd = -1;
    ...
    try
    {
        ...
        /* Transpose */
        if (libplinkio_bed_transpose_fd_(original_fd, transposed_fd, num_loci, num_samples) != PIO_OK) throw;
        if (lseek(transposed_fd, 0, SEEK_SET) == -1) throw; 
        ...
        if (bed_file->read_buffer == NULL) throw;

        transposed_fp = fdopen(transposed_fd, "w+b");
        if (transposed_fp == NULL) throw;
        transposed_fd = -1;
        fclose(bed_file->fp);
