...
def make_admixtured_population_v1( afrpop, easpop, eurpop, mixpop, n_offspr ):
    ...
    # this function implements very detailed and most complex (regarding the aiming task) plan

    # suppose we want the same number of individuals in MIX population as in
    # other three "pure" populations at the start of simulation (raunds of changing
    # generations of random mating); therefore, at first, we define how many females
    # and males have to be selected from each "pure" population,
    # a number of males should be at least the same as the number of females:
    ...
    gr_afr_males = evogen.Group() # group for selected males from afr_pop
    gr_afr_females = evogen.Group() # group for selected males from afr_pop
    gr_eas_males = evogen.Group() # group for selected males from eas_pop
    gr_eas_females = evogen.Group() # group for selected males from eas_pop
    gr_eur_males = evogen.Group() # group for selected males from eur_pop
    gr_eur_females = evogen.Group() # group for selected males from eur_pop

    # steps: (1) adding populations to the specific group
    #        (2) selecting required females
    #        (3) selecting required males
    ...
    # do the same for other populations as well:
    ...
    # checking the actual number of selected individuals:
    ...
    # making three distinct mating groups where females from one population
    # will mate with males from the other two populations:

    # to increase probability that all males contribute equally to future generation
    # (which will be moved to MIX population) we devide selected males from each population into two sub-groups:

    gr_afr_males2 = evogen.Group() # will hold hlf of selected males from AFR population
    ...
    # moving all affspring into admixture populadtion:

    off_gr = evogen.Group() # creating the group that will hold all offsprings

    # relocate offsprings into distinct group:

    gr_afr_females.regroup_newborn(off_gr)
    gr_eas_females.regroup_newborn(off_gr)
    gr_eur_females.regroup_newborn(off_gr)

    # note: untill this moment, all individuals and their offsprings belong to their original
    #       populations (the newborns belong to populations of their mothers); the next
    #       operation change this state for all individuals in the off_gr group,
    #       they will belong to the new population (MIX) in terms of location and data storage.
    
    off_gr.move(mixpop) # finally "physically" moving all offsprings into mix_pop

    # Note: we do not need to clean groups here because they are defined within the local scope of the function
    #       and there is no loop where any group is used.
...
def make_admixtured_population_v3( afrpop, easpop, eurpop, mixpop, n_offspr ):
    ...
    # we'll have only one mating group:

    mating_gr = evogen.Group()
...
# -------------------------------------------------------------------------
def simulate_admixture_pop(n_gen, n_offsprings, create_mix_mode):
    ...
    afr_pop = evogen.Population()
    eas_pop = evogen.Population()
    eur_pop = evogen.Population()
    mix_pop = evogen.Population()
    ...
    afr_pop.set_population("AFR.ssv", "map3.csv", False);
    eas_pop.set_population("EAS.ssv", "map3.csv", False);
    eur_pop.set_population("EUR.ssv", "map3.csv", False);
    ...
    for gen in range( 1, n_gen+1 ):
        ...
        # the technical part: memory management; this is optional, though recomended for large-scale simulations,
        # where many individuals frequently removed from a population;
        # though, it is NOT reccomended to reshape at every generation

        if gen % 15 == 0: # do this every 15-th iteration, the reshaping frequency depends ...
            print("Reshaping ...")
            afr_pop.reshape()
            eas_pop.reshape()
            eur_pop.reshape()
...
def main():
    ...
    generations = 10
    num_offspring = 2
    generate_mix_pop_mode = 1

    simulate_admixture_pop( generations, num_offspring, generate_mix_pop_mode )
