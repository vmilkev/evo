...
n_snp = 400; % num of all snps
n_blocks = 10; % num of haplotype blocks or recombination hot-spots
...
no_corr_fraction = 0.0; % uncorrelated fraction of a block
...
ihp = floor(n_snp_blk*0.5); % some number of haplotypes in one snp column is one block
...
scaling_range = floor(ihp/2);
t_var = exp(-0.1*first_half');
vert_shifts(first_half,1) = floor(rescale(t_var,1,scaling_range));
t_var = exp(0.1*second_half');
vert_shifts(second_half,1) = floor(rescale(t_var,1,scaling_range));
all_ihp = 0;
